    ...
    public static class StreamExtensions
    {
        ...
        public static bool ReadBoolean(this Stream stream, BooleanDataFormat format = BooleanDataFormat.Byte)
        {
            return format switch
            {
                ...
                _ => throw new ArgumentException(string.Format("Invalid {0}.", "BooleanDataFormat"), nameof(format)),
            };
        }
          ...
          ByteConverter converter = null)
        {
            return format switch
            {
                ...
                DateTimeDataFormat.CTime => StreamExtensions._cTimeBase.AddSeconds(stream.ReadUInt32(converter)),
                DateTimeDataFormat.CTime64 => StreamExtensions._cTimeBase.AddSeconds(stream.ReadInt64(converter)),
                _ => throw new ArgumentException(string.Format("Invalid {0}.", "DateTimeDataFormat"), nameof(format)),
            };
        }
          ...
          ByteConverter converter = null)
        {
            ...
            return format switch
            {
                StringDataFormat.DynamicByteCount => ReadStringWithLength(stream, Read7BitEncodedInt32(stream), false, encoding),
                StringDataFormat.ByteCharCount => ReadStringWithLength(stream, stream.ReadByte(), true, encoding),
                StringDataFormat.Int16CharCount => ReadStringWithLength(stream, stream.ReadInt16(converter), true, encoding),
                StringDataFormat.Int32CharCount => ReadStringWithLength(stream, stream.ReadInt32(converter), true, encoding),
                StringDataFormat.ZeroTerminated => ReadStringZeroPostfix(stream, encoding),
                _ => throw new ArgumentException(string.Format("Invalid {0}.", "StringDataFormat"), nameof(format)),
            };
        }
            ...
            lock (stream)
            {
                switch (format)
                {
                    ...
                    case StringDataFormat.ByteCharCount:
                        ...
                        break;
                    case StringDataFormat.Int16CharCount:
                        converter.GetBytes((short)value.Length, Buffer, 0);
                        stream.Write(Buffer, 0, 2);
                        stream.Write(bytes, 0, bytes.Length);
                        break;
                    case StringDataFormat.Int32CharCount:

...
namespace GotaSoundIO.Sound
{
    ...
    public class AudioData
    {
        ...
        /// </summary>
        public List<List<IAudioEncoding>> Channels = new();
        ...
        public void MixToMono(bool[] mutes = null)
        {
            ...
            //Test.
            if (Channels.Count == 0 || !mutes.Where(x => x == false).Any()) { return; }
        ...
        public void MixToStereo(bool[] mutes = null, bool[] isRightChannel = null, bool[] isBoth = null)
        {
            ...
            //Test.
            if (Channels.Count == 0 || !mutes.Where(x => false).Any()) { return; }
            ...
            //Start mixing.
            List<IAudioEncoding> left = new();
            List<IAudioEncoding> right = new();
            for (int i = 0; i < NumBlocks; i++)
            {
                List<float> samplesL = new();
                List<float> samplesR = new();

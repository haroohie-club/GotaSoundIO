...
namespace GotaSoundIO.Sound
{
    ...
    public class ImaAdpcm : IAudioEncoding
    {
        ...
        public List<IAudioEncoding> ChangeBlockSize(List<IAudioEncoding> blocks, int newBlockSize)
        {
            ...
            AudioData a = new() { Channels = new List<List<IAudioEncoding>>() { blocks } };
    ...
    public class ImaAdpcmDecoder
    {
        ...
        {
            short sample = this.GetSample((byte)((int)_data[Offset] >> (SecondNibble ? 4 : 0) & 15));
    ...
    public class ImaAdpcmEncoder
    {
        ...
        {
            _sample = sample = ConvertFloat(data[0]);
            _index = index = GetBestTableIndex((ConvertFloat(data[1]) - ConvertFloat(data[0])) * 8);
            _data = data;
        }
        ...
        public byte[] Encode()
        {
            byte[] data = new byte[_data.Length / 2 + (_data.Length % 2 != 0 ? 1 : 0)];
            bool secondNibble = false;
            int dataPtr = 0;
            for (int i = 0; i < _data.Length; i++)
            {
                int config = GetBestConfig(_index, ConvertFloat(_data[i]) - _sample);
                _sample = ImaAdpcmMath.ClampSample(_sample + (ImaAdpcmMath.StepTable[_index] / 8 + ImaAdpcmMath.StepTable[_index] / 4 * (config & 1) + ImaAdpcmMath.StepTable[_index] / 2 * (config >> 1 & 1) + ImaAdpcmMath.StepTable[_index] * (config >> 2 & 1)) * ((config >> 3 & 1) == 1 ? -1 : 1));
                _index = ImaAdpcmMath.ClampIndex(this._index + ImaAdpcmMath.IndexTable[config & 7]);
                if (!secondNibble)
                {
